...
test("OpenRouter response validator accepts exactly the requested meals", () => {
  const result = response.validateResponse(JSON.stringify({ meals: [
    { meal: "breakfast", name: "Omelette", reason: "Quick and familiar.", difficulty: "easy", prepMinutes: 5, cookMinutes: 10, ingredients: ["4 eggs", "100 g spinach"], existingDishId: null },
    { meal: "dinner", name: "Salmon", reason: "Matches fish night.", difficulty: "medium", prepMinutes: 10, cookMinutes: 20, ingredients: ["500 g salmon", "1 lemon"], existingDishId: "dish123" },
  ] }), ["breakfast", "dinner"]);
...
test("OpenRouter response validator rejects missing, duplicate, and malformed meals", () => {
  ...
  assert.throws(() => response.validateResponse({ meals: [
    { meal: "dinner", name: "A", reason: "Fine", difficulty: "easy", prepMinutes: 1, cookMinutes: 1, ingredients: ["A"], existingDishId: null },
    { meal: "dinner", name: "B", reason: "Fine", difficulty: "easy", prepMinutes: 1, cookMinutes: 1, ingredients: ["B"], existingDishId: null },
  ] }, ["lunch", "dinner"]), /invalid_ai_response/);
  assert.throws(() => response.validateResponse({ meals: [
    { meal: "dinner", name: "A", reason: "Fine", difficulty: "instant", prepMinutes: -1, cookMinutes: 1, ingredients: [], existingDishId: null },
