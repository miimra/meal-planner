...
test("Telegram activity identifies a person, group, and message safely", () => {
  const update = { message: { from: { id: 111, first_name: "Alex" }, text: "Hello\nfamily" } };
  assert.equal(activity.person(update), "Alex");
  assert.equal(activity.place({ type: "group", title: "Home" }), "Home");
